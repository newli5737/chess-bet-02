...
import { Loader2, Swords, ChevronLeft, LogIn } from 'lucide-react';
      ...
      <div className="w-full max-w-md relative z-10 animate-slide-up">
        <div className="glass-dark p-8 sm:p-10 rounded-3xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl glass-gold mb-4 shadow-gold animate-glow-pulse">
              <Swords className="w-8 h-8 text-amber-400" />
            </div>
            <h1 className="text-3xl font-black italic tracking-tight gradient-gold mb-2">
              ĐĂNG NHẬP
            </h1>
            <p className="text-white/50 text-sm">Chào mừng trở lại đấu trường sinh tử</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            <div className="space-y-1.5">
              ...
              <input 
                type="email" 
                required
                className="w-full bg-[hsl(220,20%,2%)]/80 border border-white/10 rounded-xl px-5 py-3.5 focus:outline-none focus:border-amber-500/50 focus:ring-1 focus:ring-amber-500/50 transition-all text-white placeholder-white/20 text-sm"
                placeholder="kythu@example.com"
                value={email}
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="w-full btn-shimmer text-amber-950 font-black text-sm rounded-xl px-4 py-4 mt-8 flex justify-center items-center gap-2 shadow-gold transition-transform active:scale-95"
            >
              {isLoading ? (
                <Loader2 className="animate-spin w-5 h-5 text-amber-950" />
              ) : (
                <>
                  <LogIn className="w-5 h-5 text-amber-950" />
                  VÀO BÀN CỜ
                </>
              )}
            </button>
          ...
          <div className="mt-8 pt-6 border-t border-white/10 text-center">
            <p className="text-sm text-white/40">
              Chưa có tài khoản?{' '}
              <Link href="/register" className="text-amber-400 hover:text-amber-300 font-bold ml-1 transition-colors">
                Đăng ký ngay
              </Link>
            </p>
          </div>
